...
※ 2025/5/9 に VSCode の拡張機能をダウンロードするための Chrome の拡張機能がリリースされています。
本サイトで紹介しているコマンドは、上記の活用例で紹介しているようにバッチ処理で利用できますので、
使い分けていただければと思います。
...
コマンドを実行すると vscode-server-<commit-id>.tar.gz という名称のファイルがダウンロードされます。
このファイルを VSCode のサーバとするマシンに転送し、次のコマンドを実行して展開してください。

#+BEGIN_EXAMPLE
cp vscode-server-linux-x64_<commit-id>.tar.gz ~/.vscode-server/bin
cd ~/.vscode-server/bin
tar zxvf vscode-server-linux-x64_<commit-id>.tar.gz
